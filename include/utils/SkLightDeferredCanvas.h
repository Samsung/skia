...
*/
class SK_API SkLightDeferredCanvas : public SkCanvas {
public:
...
//    static SkLightDeferredCanvas* Create(SkBaseDevice* device);
    ...
    /**
     * Enable thread safe playback in this canvas
     */
    void enableThreadSafePlayback();

    /**
     * Swap pending commands to playback queue.
     * Should be called from a main thread
     * The caller has to make sure that there is no ongoing flush task
     * scheduled in render thread for this canvas.
     */
    void prepareForThreadSafeFlush();

    /**
     * Trigger execution of all pending draw operations.
     * prepareForThreadSafeFlush() must have been called from
     * main thread before calling this function.
     */
    void threadSafeFlush();

    /**
     * Set playback canvas for a thread safe flush
     */
    void setPlaybackCanvas(SkCanvas* canvas);

    /**
     * Set playback surface for thread safe flush
     */
    void setPlaybackSurface(SkSurface* surface);

    /**
     * De-reference the playback surface and reset it
     */
    void clearPlaybackSurface();
     ...
     */
    void setMaxRecordingCommands(size_t maxCommands);
     ...
     * cache resources.  With SkLightDeferredCanvas, we do not cache
     * resources.
     */
    size_t freeMemoryIfPossible(size_t bytesToFree);
     
    /**
     * Specifies the maximum size (in bytes) allowed for a given image
     * to be rendered using the deferred canvas
     */
...
public:
    ...
    public:
        ...
        /**
         * Called before executing one or several draw commands into
         * an immediate canvas in threaded playback mode.  This means
         * once per flush when deferred rendering is enabled.  The
         * implementation should wait/block till the playback thread
         * finishes its current task
         */
        virtual void prepareForImmediateDraw() {}
...
protected:
    ...
    SkLightDeferredDevice* getDeferredDevice() const;

private:
    SkLightDeferredCanvas(SkLightDeferredDevice*);
