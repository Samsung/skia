...
bool SkLayerRasterizer::canRasterizeGPU(const SkPath& path,
    ...
    // Added ret value for compiling warnings on Debug mode.
    bool ret = pathBounds.intersect(bounds);
    if ((ret || !ret) && rasterRect)
...
bool SkLayerRasterizer::onRasterizeGPU(GrContext* context,
    ...
    if (SkMask::kComputeBoundsAndRenderImage_CreateMode == mode) {
        ...
        while ((rec = (SkLayerRasterizer_Rec*)iter.next()) != NULL) {
           ...
           drawMatrix.preTranslate(rec->fOffset.fX, rec->fOffset.fY);
           GrPaint grPaint;
           SkPaint2GrPaintShader(context, texture->asRenderTarget(), rec->fPaint, matrix, true, &grPaint);
           ...
           if (doAA) {
                ...
                grPaint.setCoverageSetOpXPFactory(SkRegion::kUnion_Op);
            }
            ...
            context->drawPath(texture->asRenderTarget(), grPaint, matrix, *pathPtr, strokeInfo);
        }
