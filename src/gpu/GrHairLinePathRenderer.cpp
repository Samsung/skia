...
bool GrHairLinePathRenderer::internalDrawPath(GrDrawTarget* target,
    ...
    if (IsStrokeHairlineOrEquivalent(*stroke, viewMatrix,
                                     &hairlineCoverage)) {
    ...
    SkAutoTUnref<const GrGeometryProcessor> gp(GrDefaultGeoProcFactory::Create(gpFlags,
                                               ...
                                               false,
                                               newCoverage));
    if (!arg.set(target, maxPts, gp->getVertexStride(), 0)) {
        fNumPts = 0;
        GrPrintf("Failed to get space for vertices!\n");
        return false;
    }

    HairLineVertex* verts = reinterpret_cast<HairLineVertex*>(arg.vertices());
...
FINISHED:
    ...
    SkRect devBounds;
    GetPathDevBounds(path, pipelineBuilder->getRenderTarget(), viewM, &devBounds);
    target->setIndexSourceToBuffer(indexBuffer);
    target->drawIndexedInstances(pipelineBuilder, gp, kLines_GrPrimitiveType, 1, vertex, vertex, &devBounds);
    fNumPts = 0;
    return true;
}
...
bool GrHairLinePathRenderer::onDrawPath(GrDrawTarget* drawTarget,
                                        ...
                                        const SkStrokeRec& stroke,
                                        bool stencilOnly) {
    return this->internalDrawPath(drawTarget, pipelineBuilder, color, viewMatrix, path, stroke, false);
}

GrPathRenderer::StencilSupport GrHairLinePathRenderer::onGetStencilSupport(const GrDrawTarget*,
