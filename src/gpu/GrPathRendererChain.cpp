...
GrPathRenderer* GrPathRendererChain::getPathRenderer(const SkPath& pathA,
    ...
    for (int i = 0; i < fChain.count(); ++i) {
        ...
        if (fChain[i]->canDrawPath(pathA, pathB, pathC, stroke, target, pipelineBuilder, color, viewMatrix, antiAlias)) {
            if (GrPathRenderer::kNoSupport_StencilSupport != minStencilSupport) {
                GrPathRenderer::StencilSupport support = fChain[i]->getStencilSupport(target, pipelineBuilder, pathA, stroke);
                if (support < minStencilSupport) {
                    continue;
